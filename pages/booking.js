...
export default function Booking() {
  ...
  // Memoize checkRoomAvailability to avoid unnecessary re-creations
  ...
  // Allow selecting both AC and Non-AC for the same room type, but not both for the same room id
  const handleRoomSelection = (room, isAc) => {
    setSelectedRooms((prev) => {
      // Only allow one selection (AC or Non-AC) per room id
      const filtered = prev.filter((r) => r.id !== room.id);
      return [...filtered, { ...room, isAc }];
    });
  };
  ...
  return (
    <>
      <Navbar />
      <div
        className={`min-h-screen flex flex-col items-center justify-center bg-[#C6A38D] py-6 sm:py-12 px-4 sm:px-6 lg:px-8 border-l-[10px] border-r-[10px] border-b-[10px] border-[#ffffff] ${
          showConfirmationModal ? "blur-sm" : ""
        }`}
      >
        <div className="w-full max-w-3xl rounded-[50px] mx-auto bg-white shadow-lg p-4 sm:p-8 border-2 border-[#8B593E] flex flex-col items-center mt-24">
          <h2 className="text-2xl sm:text-3xl font-bold text-center mb-6 sm:mb-10 text-[#8B593E]">
            Book Your Stay Now...
          </h2>
          ...
          <form
            onSubmit={showCustomerForm ? handleBookingSubmit : handleSubmit}
          ...
          >
            {!showCustomerForm ? (
              <>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
                  <div className="space-y-2">
                    <label
                      htmlFor="numPeople"
                      className="block text-sm sm:text-base font-medium text-[#4A2511]"
                    >
                      Number of Guests
                    </label>
                    <input
                    ...
                    <div className="bg-white rounded-xl p-4 sm:p-6 w-full max-h-[90vh] overflow-y-auto">
                      ...
                      {loading ? (
                        <div className="text-center py-8">
                          ...
                          <p className="mt-4 text-lg text-[#4A2511]">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 flex-grow">
                            {availableRooms.map((room) => (
                              ...
                              >
                                ...
                                <h4 className="text-lg font-semibold text-[#8B593E] mb-2">
                                  {room.description}
                                </h4>
                                ...
                                <div className="space-y-3 mt-4">
                                  <div className="flex items-center justify-between p-3 bg-[#F2E2D7] rounded-lg">
                                    <div>
                                      ...
                                      <p className="text-lg font-bold text-[#8B593E]">
                                        ₹{room.acCost}
                                      </p>
                                    </div>
                                    <button
                            ...
                            {selectedRooms.length > 0 && (
                              <div className="lg:sticky lg:top-6">
                                <div className="p-4 bg-[#F2E2D7] rounded-xl">
                                  <h3 className="text-xl font-semibold text-[#4A2511] mb-3">
                                    Your Selection
                                  </h3>
                                  <div className="space-y-2">
                                    ...
                                    <div className="border-t-2 border-[#C6A38D] pt-3 mt-3">
                                      <div className="flex justify-between items-center text-lg font-bold">
                                        <span className="text-[#4A2511]">
                                          Total Amount:
                                        </span>
                                        <span className="text-[#8B593E]">
                                          ₹{totalPrice}
                                        </span>
                                      </div>
                                    </div>
              ...
              <div className="space-y-4 sm:space-y-6">
                <h3 className="text-xl sm:text-2xl font-semibold text-[#4A2511] text-center">
                  Your Details
                </h3>
                <div className="space-y-3 sm:space-y-4">
                  <div>
                    <label
                      ...
                      className="block text-base sm:text-lg font-medium text-[#4A2511] mb-1 sm:mb-2"
                    >
                      Email Address
                    </label>
                    <input
      ...
      {showConfirmationModal && (
        <div className="fixed inset-0 bg-[#C6A38D]/5  flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full text-center">
            <h3 className="text-2xl font-bold text-[#8B593E] mb-4">
              Booking Confirmed!
            </h3>
            <p className="text-gray-700 mb-6">
              Thank you for choosing Pearl Homestay! We have sent a confirmation
              email to {customerEmail}. We look forward to welcoming you soon.
            </p>
            <button
              onClick={() => {
